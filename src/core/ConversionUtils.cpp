 ...
 */
void Parse3DPointValues(std::vector<pmp::Scalar>& valueBuffer, const std::string& str)
{
	if (str.empty())
		return;
...
[[nodiscard]] bool ArePointValuesValid(const std::vector<pmp::Scalar>& valueBuffer)
{
...
[[nodiscard]] float GetCellSize(const std::vector<pmp::Scalar>& spacingVec)
{
...
Geometry::ScalarGrid ImportVTI(const std::string& fileName)
{
	...
	std::vector<pmp::Scalar> originPtCoords{};
	...
	std::vector<pmp::Scalar> cellSpacing{};
	...
	// >>>>>>> compute box <<<<<<<<<<<<<<
	constexpr pmp::Scalar boxEpsilon = 1e-6; // round-off error for ceil/floor in global grid coords.
