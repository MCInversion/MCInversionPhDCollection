...
namespace Geometry
{
	...
	bool Export3DPointCloudToPLY(const std::vector<pmp::Point>& points, const std::string& fileName)
	{
		const auto extension = Utils::ExtractLowercaseFileExtensionFromPath(fileName);
		if (extension != "ply")
		{
	...
	bool ExportBaseCurveGeometryDataToPLY(const Geometry::BaseCurveGeometryData& geomData, const std::string& absFileName)
	{
