...
namespace Geometry
{
	...
	[[nodiscard]] static std::pair<Tetrahedron, std::vector<pmp::Point>> FindInitialTetrahedron(const std::vector<pmp::Point>& points)
	{
		std::vector minMax(6, points[0]); // Initialize with the first point
		for (const auto& point : points)
		{
			if (point[0] < minMax[0][0]) minMax[0] = point;
			if (point[1] < minMax[1][1]) minMax[1] = point;
			if (point[2] < minMax[2][2]) minMax[2] = point;
			if (point[0] > minMax[3][0]) minMax[3] = point;
			if (point[1] > minMax[4][1]) minMax[4] = point;
			if (point[2] > minMax[5][2]) minMax[5] = point;
		}
		...
		for (int i = 0; i < 6; ++i)
		{
			for (int j = i + 1; j < 6; ++j)
			{
				const pmp::Scalar distSq = sqrnorm(minMax[i] - minMax[j]);
				if (distSq > maxDistSq)
				{
					maxDistSq = distSq;
					tetra.Point0 = minMax[i];
					tetra.Point1 = minMax[j];
				}
			}
		}
		...
		for (const auto& point : points)
		{
			const pmp::Scalar area = norm(cross(tetra.Point1 - tetra.Point0, point - tetra.Point0));
			if (area > maxDistSq)
			{
				maxDistSq = area;
				tetra.Point2 = point;
		...
		maxDistSq = -FLT_MAX;
		const pmp::Normal planeNormal = normalize(cross(tetra.Point1 - tetra.Point0, tetra.Point2 - tetra.Point0));
		const pmp::Scalar planeDist = dot(planeNormal, tetra.Point0);
		for (const auto& point : points)
		{
			const pmp::Scalar dist = std::abs(dot(planeNormal, point) - planeDist);
			if (dist > maxDistSq)
			{
				maxDistSq = dist;
				tetra.Point3 = point;
		...
		std::vector<pmp::Point> remainingPoints;
		for (const auto& point : points)
		{
			if (point != tetra.Point0 && point != tetra.Point1 && point != tetra.Point2 && point != tetra.Point3)
			{
				remainingPoints.push_back(point);
			}
		}

		return { tetra, remainingPoints };
	}

	static void InitializeFromTetrahedron(pmp::SurfaceMesh& mesh, const Tetrahedron& tetrahedron)
	{
		// Add points to the mesh
		const auto v0 = mesh.add_vertex(tetrahedron.Point0);
		const auto v1 = mesh.add_vertex(tetrahedron.Point1);
		const auto v2 = mesh.add_vertex(tetrahedron.Point2);
		const auto v3 = mesh.add_vertex(tetrahedron.Point3);
	...
	static [[nodiscard]] std::pair<PointConstIterator, pmp::Scalar> FindFarthestPoint(const std::vector<pmp::Point>& points, const pmp::SurfaceMesh& mesh)
	{
	...
	static void UpdateVisibilityData(const std::vector<pmp::Point>& inputPoints,
		...
		const std::unordered_set<unsigned int>& visited)
	{
		...
		for (const auto f : convexHull.faces())
		{
			...
			{
				if (visited.contains(i)) // Skip if visited
				{
					continue;
				}
	...
	static void UpdateConvexHull(pmp::SurfaceMesh& mesh, const pmp::Point& newPoint)
	{
		const auto vNew = mesh.add_vertex(newPoint);

		// 1. Identify Faces Visible from newPoint
		std::vector<pmp::Face> visibleFaces;
		for (const auto f : mesh.faces())
		{
			auto fVit = mesh.vertices(f);
			auto normal = pmp::Normals::compute_face_normal(mesh, f);
			auto vec = newPoint - mesh.position(*fVit);
			if (dot(normal, vec) > 0) // Visible face
			{
				visibleFaces.push_back(f);
			}
		}
		const std::set visibleFacesSet(visibleFaces.begin(), visibleFaces.end());

		// 2. Remove Visible Faces
		//    and identify boundary edges
		std::vector<pmp::Edge> bdEdges;
		bdEdges.reserve(visibleFaces.size());
		for (const auto f : visibleFaces)
		{
			for (const auto h : mesh.halfedges(f))
			{
				const auto oppHe = mesh.opposite_halfedge(h);
				if (visibleFacesSet.contains(mesh.face(oppHe)))
					continue;

				bdEdges.push_back(mesh.edge(oppHe));
			}
			mesh.delete_face(f);
		}

		// 3. Triangulate the Hole with the new vertex
		for (const auto bdE : bdEdges)
		{
			const auto h = mesh.halfedge(bdE, 0);
			const auto start = mesh.from_vertex(h);
			const auto end = mesh.to_vertex(h);
			mesh.add_triangle(vNew, start, end);
		}

		mesh.garbage_collection(); // Clean up deleted faces
	}
	...
	std::optional<pmp::SurfaceMesh> ComputePointCloudConvexHull(const std::vector<pmp::Point>& points, const pmp::Scalar& distTolerance)
	{
		...
		{
			// The resulting hull does not meet the minimum vertex requirement
			return {};
		}
