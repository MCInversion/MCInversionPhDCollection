...
namespace MAT
{
	...
	std::vector<BoundaryElement> BoundaryGenerator::getBoundaryElementsFromCurve(const std::vector<Vector2d>& customShapePoints)
	{
		boundaryElements.clear();
		generateShape(customShapePoints.data(), static_cast<int>(customShapePoints.size()));
		return boundaryElements;
	}
